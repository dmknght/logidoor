...
from logidoor.old.libs.cores.actions import rand_from_list, get_domain
from logidoor.old.libs.cores.check import find_login_form
from logidoor.old.libs.cores.analysis import check_sqlerror, get_redirection
...
def submit(options, login_field, tryCred, result):
	...
	try:
		...
		if options.proxy:
			# Set proxy connect
			proxy_address = rand_from_list(options.proxy)
